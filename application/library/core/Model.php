...
abstract class Model {
	 ...
	 * Orderby
	 */
	final public function Orderby($sort){
		if(is_string($sort)){
			$this->options['orderby'] = $sort; // 2:　Cos there is a BLANK
		}else{
			$this->options['orderby'] = $sort;
		}
		return $this;
	}
	 ...
	 * Lock
	 */
	final public function Lock($falg){
		if($falg){
			$this->options['Lock'] = ' for update'; // 2:　Cos there is a BLANK
		}
	...
	***********/
	 ...
	 * @return FALSE on failure or inserted_id on success
	 */
	final public function Insertsetfield($map = array()) {
		if (!$map || !is_array($map)) {
			return FALSE;
		} else {
			$fields = $values = array();
			...
			$this->sql = "INSERT INTO " . $this->table . " SET $field".';';
	 ...
	 * @return boolean
	 */
    final public function MultiInsert($data){
     ...
     * @return array
//     */
//    public function pagelimit($pageSize=10,$currentPage=1,$where=null){
//        $currentRow=($currentPage-1)*$pageSize;
//        $totalCount=$this->Total();
////        if($where){
////            $data=array('data'=>$this->between('id',$currentRow+1,$currentRow+$pageSize)->where($where)->Select(),'total'=>$totalCount,'current'=>$currentPage,'rowCount'=>$pageSize);
////        }else{
////            $data=array('data'=>$this->between('id',$currentRow+1,$currentRow+$pageSize)->Select(),'total'=>$totalCount,'current'=>$currentPage,'rowCount'=>$pageSize);
////        }
////        select * from wl_admin where `id` in (select t.id from (select `id` from wl_admin limit 0,10) as t)
//        $this->order()
//
//        return $data;
//    }
    ...
    /** 模糊查询分页
     * @param int $rowCount 显示条数
     * @param int $current 当前页
     * @param string $sort 升降序
     * @param null $like 模糊查询
     * @return array
     */
	final public  function tabPage($rowCount,$current,$sort='id desc',$like=null,$where=null){
		if(!isset($this->primary) || empty($this->primary)){
			$this->primary='id';
		}
        if(isset($this->options['field'])){
            $field = $this->options['field'];
        ...
        $startCount=($current-1)*$rowCount; //开始limit
        $endCount=$startCount+$rowCount;//结束limit

        if($field!='*'){
            $fields=explode(',',$field);
            $str='';
            foreach ($fields as $item){
                $str.= ' or '.$item.' like '.' "%'.$like.'%" ';
            }
            $str=substr($str,3,strlen($str));
        }
		
		/*if($like){
            $tsql='(select t.id from (select '.TB_PK.' from `'. $this->table. '` where '.$str.($where!=null?' and '.$where:' ').' order by `'.array_keys($sort)[0].'` '.$sort[array_keys($sort)[0]].' limit '.$startCount.','.$rowCount.')as t)'.' order by `'.array_keys($sort)[0].'` '.$sort[array_keys($sort)[0]];
        }else{

            $tsql='(select t.id from (select '.TB_PK.' from `'. $this->table.'`'. ($where!=null?' where '.$where:' ').' order by `'.array_keys($sort)[0].'` '.$sort[array_keys($sort)[0]]. ' limit '.$startCount.','.$rowCount.')as t)'.' order by `'.array_keys($sort)[0].'` '.$sort[array_keys($sort)[0]];
        }*/
		if(is_array($sort)){
			if($like){
				$tsql='(select '.$this->primary.' from (select '.$this->primary.' from `'. $this->table. '` where '.$str.($where!=null?' and '.$where:' ').' order by `'.array_keys($sort)[0].'` '.$sort[array_keys($sort)[0]].' limit '.$startCount.','.$rowCount.')as t)';
				if(isset($this->options['conditions'])){
					$tsql.= $this->options['conditions'];
				}
				$tsql.=' order by `'.array_keys($sort)[0].'` '.$sort[array_keys($sort)[0]];
			}else{
				$tsql='(select '.$this->primary.' from (select '.$this->primary.' from `'. $this->table.'`'. ($where!=null?' where '.$where:' ').' order by `'.array_keys($sort)[0].'` '.$sort[array_keys($sort)[0]]. ' limit '.$startCount.','.$rowCount.')as t)';
				if(isset($this->options['conditions'])){
					$tsql.= $this->options['conditions'];
				}
				$tsql.=' order by `'.array_keys($sort)[0].'` '.$sort[array_keys($sort)[0]];
			...
			if($like){
				$tsql='(select '.$this->primary.' from (select '.$this->primary.' from `'. $this->table. '` where '.$str.($where!=null?' and '.$where:' ').' order by '.$sort.' limit '.$startCount.','.$rowCount.')as t)';
				if(isset($this->options['conditions'])){
					$tsql.= $this->options['conditions'];
				}
				$tsql.=' order by  '.$sort;
			}else{
				$tsql='(select '.$this->primary.' from (select '.$this->primary.' from `'. $this->table.'`'. ($where!=null?' where '.$where:' ').' order by '.$sort. ' limit '.$startCount.','.$rowCount.')as t)';
				if(isset($this->options['conditions'])){
					$tsql.= $this->options['conditions'];
				}
				$tsql.=' order by  '.$sort;
        ...
        $sql = 'SELECT '. $field .' FROM `'. $this->table. '` where `'.$this->primary.'` in ';
        ...
        $this->selectOne = FALSE;
        if($like) {
            $countSql = 'SELECT count(*) as `total` FROM `' . $this->table . '` where '.$str.($where!=null?' and '.$where:' ');
        }else{
            $countSql = 'SELECT count(*) as `total` FROM `' . $this->table . '`'.($where!=null?' '.' where '.$where:' ');
        }
//        echo $countSql;exit;
        $this->sql=$countSql;
        $this->connect('READ');
        ...
        $this->selectOne = FALSE;
        return array('data'=>$data,'current'=>$current,'rowCount'=>$rowCount,'total'=>$totalCount[0]['total']);
    }
	...
	final public function Update($map, $self = FALSE) {
		...
		if (!$map) {
			...
			if(isset($this->options['conditions'])){
				$this->sql .= $this->options['conditions'];
			}
	...
	public function Locktable() {
		self::$conn->query("lock table ".$this->table." write");
	}
