...
abstract class Model {
	...
	***********/
	 ...
	 *
	 * @param string => Group by field
	 * @return int => Group by field
	 */
...
//    }
	...
	final public  function tabPage($rowCount,$current,$sort='id desc',$like=null,$where=null){
		...
		if(is_array($sort)){
			if($like){
				$tsql='(select t.id from (select '.TB_PK.' from `'. $this->table. '` where '.$str.($where!=null?' and '.$where:' ').' order by `'.array_keys($sort)[0].'` '.$sort[array_keys($sort)[0]].' limit '.$startCount.','.$rowCount.')as t)';
				if(isset($this->options['conditions'])){
					$tsql.= $this->options['conditions'];
				}
				$tsql.=' order by `'.array_keys($sort)[0].'` '.$sort[array_keys($sort)[0]];
			}else{
				$tsql='(select t.id from (select '.TB_PK.' from `'. $this->table.'`'. ($where!=null?' where '.$where:' ').' order by `'.array_keys($sort)[0].'` '.$sort[array_keys($sort)[0]]. ' limit '.$startCount.','.$rowCount.')as t)';
				if(isset($this->options['conditions'])){
					$tsql.= $this->options['conditions'];
				}
				$tsql.=' order by `'.array_keys($sort)[0].'` '.$sort[array_keys($sort)[0]];
			...
			if($like){
				$tsql='(select t.id from (select '.TB_PK.' from `'. $this->table. '` where '.$str.($where!=null?' and '.$where:' ').' order by '.$sort.' limit '.$startCount.','.$rowCount.')as t)';
				if(isset($this->options['conditions'])){
					$tsql.= $this->options['conditions'];
				}
				$tsql.=' order by  '.$sort;
			}else{
				$tsql='(select t.id from (select '.TB_PK.' from `'. $this->table.'`'. ($where!=null?' where '.$where:' ').' order by '.$sort. ' limit '.$startCount.','.$rowCount.')as t)';
				if(isset($this->options['conditions'])){
					$tsql.= $this->options['conditions'];
				}
				$tsql.=' order by  '.$sort;
        ...
        $sql = 'SELECT '. $field .' FROM `'. $this->table. '` where `'.TB_PK.'` in ';
...
//        echo $countSql;exit;
	...
	final public function Counts($field) {
